...
static void dump_menu_state() {
    ...
    LOGI("--- TABS ---");
    LOGI("[Player] [Movement] [Visuals] [Players] [Spawner] [Config]");
...
static void try_spawn_selected() {
    ...
    // Aquí se conecta al método real de spawn del juego cuando tengas el MethodInfo
}
...
static void on_load() {
    LOGI("liblar_menu.so loaded — Spawner + AntiKick ON");
    pthread_t t;
